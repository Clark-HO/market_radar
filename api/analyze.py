...
# [Debug Wrapper] Capture Import Errors (e.g., google-genai missing)
import_error = None
client = None
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        # 1. Setup Headers (Always 200 to debug)
        ...
        if import_error:
             self.wfile.write(json.dumps({
                ...
                "report": f"⚠️ Server Import Failed: {import_error}"
            }).encode('utf-8'))
             return
        ...
        try:
            ...
            # If stock_id is N/A, maybe just testing endpoint
            if stock_id == "N/A":
                self.wfile.write(json.dumps({
                    "score": 100, "verdict": "System Online", "report": "✅ API is ALIVE. Please provide stock_id."
                }).encode('utf-8'))
                return
            ...
            verdict = verdict_match.group(1).strip() if verdict_match else "AI Analysed"

            out = { "score": score, "verdict": verdict, "report": content }
            self.wfile.write(json.dumps(out).encode('utf-8'))

        except Exception as e:
            self.wfile.write(json.dumps({
                "score": 0, "verdict": "System Error", 
                "report": f"⚠️ Runtime Error: {str(e)}"
            }).encode('utf-8'))
